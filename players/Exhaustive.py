...
class Exhaustive (Player):
    ...
    def preprocessing ( self:       Self,
        ...
        self.best_permutation = self.find_best_permutation(game_state)


        # Find the whole way
        self.way = self.permutation_to_way(self.best_permutation, self.simplified_graph)
...
#####################################################################################################################################################
    ...
    def distance_of_way( self: Self,
                        way: List[Integral],
                      ) -> float:
        """
            This method returns the distance of a way in a graph.
            In:
                * way:   The way.
            Out:
        ...
        for i in range(1, len(way)):
            distance += self.simplified_graph[way[i-1]][way[i]]["distance"]
        return distance
    ...
    def find_best_permutation( self: Self,
                              game_state: GameState
                              ) -> List[Integral]:
        """
            This method returns the best permutation of cheeses to visit.
            In:
                * game_state: The state of the game.
            Out:
                * The best permutation
        """
        # List all the permutations of the cheeses
        self.permutations = list(permutations(game_state.cheese))

        # Add the source to the permutations
        self.permutations_with_source = [[game_state.player_locations[self.name]] + list(permutation) for permutation in self.permutations]

        best_permutation = None
        best_distance = float('inf')

        # Test all permutations
        for permutation in self.permutations_with_source:
            distance = self.distance_of_way(permutation)

            # If better, update the best permutation
